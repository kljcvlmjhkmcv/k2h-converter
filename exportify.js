...
const utils = {
	...
	async authorize() { // This is bound to the login button in the HTML and gets called when the login button is clicked.
		...
		location = "https://accounts.spotify.com/authorize?client_id=f3fd2e13bd8046e4b801f01e43b4500b" +
			"&redirect_uri=" + encodeURIComponent("https://www.virtualdz.store/") +
...
onload = async () => {
	...
	if (code) {
		...
		let response = await fetch("https://accounts.spotify.com/api/token", { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'},
			body: new URLSearchParams({client_id: "f3fd2e13bd8046e4b801f01e43b4500b", grant_type: 'authorization_code', code: code, redirect_uri: "https://www.spotmyplaylist.site/",

...
async function copyTracks(sourcePlId, targetPlId, sourceToken, targetToken) {
  ...
  while (url) {
    ...
    if (uris.length > 0) {
      const post = await fetch(`https://api.spotify.com/v1/playlists/${targetPlId}/tracks`, {
        method: "POST",
        headers: {
...
async function transferLikedSongs(source, target) {
  ...
  while (url) {
    ...
    if (uris.length > 0) {
      const put = await fetch("https://api.spotify.com/v1/me/tracks", {
        method: "PUT",
        headers: {
...
async function transferSavedAlbums(source, target) {
  ...
  while (url) {
    ...
    if (ids.length > 0) {
      const put = await fetch("https://api.spotify.com/v1/me/albums", {
        method: "PUT",
        headers: {
          Authorization: "Bearer " + target,
          "Content-Type": "application/json"
...
async function transferSavedPodcasts(source, target) {
  ...
  while (url) {
    ...
    if (ids.length > 0) {
      const put = await fetch("https://api.spotify.com/v1/me/shows", {
        method: "PUT",
        headers: {
          Authorization: "Bearer " + target,
          "Content-Type": "application/json"
